...
export default function NewDrive({ isOpen, onClose }) {
  ...
  const [currentUser, setCurrentUser] = useState("test"); // 입력 필드 값
  ...
  useEffect(() => {
   ...
   const handleSelectMaster = () => {
    if (!currentUser.trim()) return; // 값이 없으면 설정하지 않음
    setFormData({
      ...formData,
      driveMaster: currentUser, // 선택된 이름
      masterEmail: `${currentUser.toLowerCase().replace(/\s/g, ".")}@example.com`, // 예시 이메일 생성
    });
    setCurrentUser(""); // 검색 필드 초기화
  };
