...
export const ProjectTaskDynamic = ({
  ...
  const handleAddSubTask = () => {
    if (newSubTask.trim()) {
      ...
      const randomId = Math.random().toString(36).substr(2, 9); // 랜덤한 9자리 문자열
  
      // 새로운 하위 목표 추가
      setNowSubTasks([...nowSubTasks, { id: randomId, name: newSubTask, isChecked: false }]);
      ...
      {isSelected ? (
        <div className="flex flex-col flex-1 w-full">
          ...
          <div className="flex items-start py-px gap-2">
            <div className="w-[20px]" aria-label="Priority">
          ...
          {/* SubTasks Section */}
          <section className="flex flex-col mt-1.5" aria-labelledby="subtasks">
            {nowSubTasks.map((subTask, index) => (
              ...
              >
                <input
                  ...
                  aria-label={`Subtask ${subTask.name}`}
                />
                <label
                  htmlFor={`check${index}`}
          ...
          <section
            ...
            aria-labelledby="due-date-comments"
          >
            {duedate && (
              <div
        ...
        <>
          ...
          <div className="flex flex-col flex-1 shrink basis-0 text-black text-opacity-50">
            ...
            {content && (
              <div
                className={clsx("max-w-[200px] leading-4 truncate text-xs", {
                  "line-through": status !== "active",
                })}
              >
                {content}
